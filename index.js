...
function nameCorrection(array, index, artistName) {
  array[index].name = artistName;
  return array;
}
console.log(nameCorrection(artists, 8, "Vincent Van Gogh"));
...
function getArtistByIndex(artistsArray, index) {
  getArtistByIndexResult = artistsArray[index].name;
  return `The artist at index ${index} is ${getArtistByIndexResult}`
}
console.log(getArtistByIndex(artists, 14))
...
*/
function removeArtist(artistsArray, idnex) {
  artistsArray[index].splice;
  return artistsArray.length-1;
}
   console.log(removeArtist(arists, 0))
...
console.log(addArtist(artists, "id: 20", "name: Cortez Elzy", "years: 1996- 10/25/20", "genre: Web Design", "nationality: American", "bio: just started learning to code. only a few months worth of experince so far."))
